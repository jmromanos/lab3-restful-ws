...
public class AddressBookServiceTest {
	...
	public void listUsers() throws IOException {
        ...
        yo.setName("Yo");
        URI yoURI = URI.create("http://localhost:8282/contacts/person/1");
        response = client.target("http://localhost:8282/contacts").request(MediaType.APPLICATION_JSON)
                .post(Entity.entity(yo, MediaType.APPLICATION_JSON));

        assertEquals(201, response.getStatus());
        assertEquals(yoURI, response.getLocation());
        assertEquals(MediaType.APPLICATION_JSON_TYPE, response.getMediaType());
        Person yoUpdated = response.readEntity(Person.class);
        assertEquals(yo.getName(), yoUpdated.getName());
        assertEquals(1, yoUpdated.getId());
        assertEquals(yoURI, yoUpdated.getHref());
